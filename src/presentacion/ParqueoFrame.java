...
public class ParqueoFrame extends JFrame {
    ...
    private JPanel crearPanelFormulario() {
        ...
        JLabel tituloSeleccion = new JLabel("Registrar salida");
    ...
    private JPanel crearPanelTabla() {
        ...
        JSplitPane splitPane = new JSplitPane(JSplitPane.VERTICAL_SPLIT,
                crearContenedorTabla("Vehículos actualmente en el parqueo", tablaActivos),
                crearContenedorTabla("Historial de vehículos", tablaHistorial));
        splitPane.setResizeWeight(0.55);
        splitPane.setBorder(null);
        panel.add(splitPane, BorderLayout.CENTER);

        JPanel pie = new JPanel(new FlowLayout(FlowLayout.LEFT, 0, 0));
        pie.add(new JLabel("La salida calcula automáticamente ₡500 por hora o fracción."));
    ...
    private void registrarSalida(ActionEvent evento) {
        ...
        if (seleccionado == null) {
            mostrarMensaje("Seleccione un vehículo activo antes de registrar la salida.", true);
            return;
        }
        try {
            RegistroParqueo registro = servicio.registrarSalida(seleccionado.getPlaca());
            cargarDatos();
            mostrarMensaje("Salida registrada para " + registro.getPlaca() + ". Monto a pagar: "
                    + String.format("₡%,d", registro.getMontoCobrado()) + ".", false);
        } catch (ParqueoException ex) {
            mostrarMensaje(ex.getMessage(), true);
        }
    }
